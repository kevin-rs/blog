...
pub fn Blog() -> Element {
    ...
    let route_count = BlogRoute::static_routes().len();
    let grid_class = match route_count {
        1 => "mb-8 grid grid-cols-1 gap-6",
        2 => "mb-8 grid grid-cols-2 gap-6",
        _ => "mb-8 grid grid-cols-3 gap-6",
    };
    rsx! {
        section {
            id: "blog",
            class: "flex flex-col items-center p-4 min-h-screen justify-center",
            Title {}
            div {
                class: "flex gap-4 mt-8 mb-8",
